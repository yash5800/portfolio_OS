...
const Container = ({children,title,id}) => {
  const {setPage,removeWindow} = React.useContext(MainContext);
  ...
  return (
    <ContainerContext.Provider value={{isMobile,setMobile:setIsMobile}}>
        <div className={` ${full ? 'h-[90%] w-[90%]' : isMobile ? `w-[300px] h-[90%]` : 'h-[90%] w-[500px]'} bg-gray-800 text-white p-2 rounded-xl flex justify-start items-center flex-col gap-2`}>
