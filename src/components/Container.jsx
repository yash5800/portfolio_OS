...
const Container = ({children,title,id}) => {
  const {setPage,removeWindow,handleDrag} = React.useContext(MainContext);
        ...
        <div className={` ${full ? 'h-[98%] w-[90%]' : isMobile ? `w-[300px] h-[90%]` : 'h-[90%] w-[500px]'} bg-gray-800 text-white p-2 rounded-xl flex justify-start items-center flex-col gap-2`}>
          <div className={`flex justify-between items-center border-b border-gray-600 px-3 w-full top-0 relative`}>
            ...
            <div className='absolute opacity-0 w-full top-0 left-0 hover:cursor-grab active:cursor-grabbing'
             ...
             ><p>grab</p></div>
            <div className='flex justify-center items-center gap-3 z-2'>
              ...
              <IoIosCloseCircle className='text-xl cursor-pointer hover:scale-110' color='red' onClick={() => removeWindow(id)} title='Close' />
            </div>
          </div>
          <div className='relative h-full w-full overflow-y-auto overflow-x-hidden overflow-scroll'>
            {children}
          </div>
         </div>
